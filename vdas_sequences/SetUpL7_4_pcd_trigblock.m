...
numFrames = 50; % number of triggerd frames

hifuOnTime_us=110;

us_between_acq=hifuOnTime_us / (numacq-1);
%us_between_acq=11;

num_final_bmode_acs=2;
us_between_bmodes=200;
...
Resource.RcvBuffer(2).rowsPerFrame = rowsPerAcq*num_final_bmode_acs;%4096*33; % fits 50+ acquisions per frame
...
Receive = repmat(struct('Apod', zeros(1,128), ...
                        ...
                        'samplesPerWave', 4, ...
                        'mode', 0, ...
                        'callMediaFunc', 1),1,numacq*Resource.RcvBuffer(1).numFrames + num_final_bmode_acs); 
...
bmode_save_buff=nn+1;
for bi = 1:num_final_bmode_acs
    nn=nn+1;
    %Recieve struct for the final b-mode acquisition
    Receive(nn).bufnum=2;
    Receive(nn).Apod(1:128) = 1.0;
    Receive(nn).framenum = 1;
    Receive(nn).acqNum = bi;
end
...
Process(3).method = 'save_data_pcd_trigV';
...
TIMETONEXTACQ=si; si=si+1;
SeqControl(TIMETONEXTACQ).command = 'timeToNextAcq';  % time between saved acquisitions
SeqControl(TIMETONEXTACQ).argument = us_between_acq; 
...
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:numacq 
        ...
        if (j==1)
            ...
            Event(n).seqControl = [nsc, SYNC_seqidx, TIMETONEXTACQ];
...
%             nsc = nsc+1;
        ...
        elseif j < numacq
            ...
            Event(n).seqControl = [TIMETONEXTACQ];
        else 
            Event(n).seqControl = 0;
        end
...
Event(n).recon = 0;      % no reconstruction.
Event(n).process = 0; 
Event(n).seqControl = [NOOP_seqidx, SYNC_seqidx];
n=n+1;
% 
% %very last acquisitions are B-mode
for bi=1:num_final_bmode_acs
    Event(n).info = 'Flash transmit';
    Event(n).tx = 1;%j;         %do T/R to get a B-mode image
    Event(n).rcv = bmode_save_buff+bi-1;    
    Event(n).recon = 0;      % no reconstruction.
    Event(n).process = 0;    % no processing
    Event(n).seqControl = nsc;
    SeqControl(nsc).command = 'timeToNextAcq';
    SeqControl(nsc).argument = us_between_bmodes; 
    nsc=nsc+1;
    n=n+1;
end
